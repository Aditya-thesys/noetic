...
/**
 * Creates a steering context layer that evaluates rules before tool calls and after model calls.
 *
 * @public
 * @param config - Steering configuration including rules, scope, and max ledger size.
 * @returns A `ContextLayer` that enforces steering rules via allow/deny/guide decisions.
 */
export function steering(config: SteeringConfig) {
  const maxLedger = config.maxLedgerEntries ?? DEFAULT_MAX_LEDGER_ENTRIES;
  const maxRetries = config.maxRetries ?? 1;
  const scope: ContextScope = config.scope ?? 'execution';
  ...
  } satisfies ContextLayer<SteeringState>;
}
