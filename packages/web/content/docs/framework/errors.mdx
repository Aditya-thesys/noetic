...
| `cancelled` | `reason?: string` | The execution was cancelled via `ctx.abort()` or `harness.cancel()` — including a parent's cancellation cascading into a `fork` path or `spawn` child. Pending channel `recv` calls (and parked back-pressure `send` calls) on the aborted context reject with this kind immediately, and an in-flight `llm` call or sub-harness turn is cut short. |
...
- [Context & Event Log](/docs/framework/context) -- `ctx.abort()` produces a `cancelled` error.
- [Channels](/docs/framework/operators/channels) -- `channel_timeout` and `channel_closed` errors.
- [Loop & Until](/docs/framework/operators/loop-and-until) -- the `onError` recovery callback.
- [AgentHarness](/docs/framework/runtime) -- `harness.cancel()` triggers cancellation errors.
