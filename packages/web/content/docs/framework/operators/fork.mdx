...
Each path is a child execution. It inherits the parent's memory layers and tool pool, so `llm` steps inside a path keep their memory projection and layer tools, and a nested `spawn` has layers to inherit.

Layer *state* is per path:

- `onSpawn` seeds each path's state from the parent's before the path runs. Items returned by `onSpawn` are not appended -- a fork child already has the parent's full item log.
...
For layers that collect artifacts from many workers, choose a merge strategy that namespaces per child -- see [`durableTaskState({ mergeData: 'namespace' })`](/docs/framework/memory/durable-task-state).
