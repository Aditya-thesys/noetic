...
`parseAndRunWorkflow` opens a root `workflow.run` span that carries the static workflow graph — the *potential paths* of the DAG, independent of which branches actually run. Model and tool spans nest under it, so the exported trace mirrors the declared workflow with the executed path overlaid.
