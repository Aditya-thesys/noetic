...
| Layer | Placement |
|---|---|
| [`staticContent`](/docs/framework/context-layers/static-content) | `'anchor'` |
| [`fileReference`](/docs/framework/context-layers/file-reference) | `'anchor'`, and implements `renderDelta` |
| [`steering`](/docs/framework/context-layers/steering) | `'live'` |
| [`temporalContext`](/docs/framework/context-layers/temporal-context) | `'live'` with `groundDateTime`, otherwise `'auto'` |
...
<update layer="file-reference" action="replace">
...
</update>
</context_updates>
```
...
Cap history with [`historyWindow`](/docs/framework/context-layers/history-window) so the drop happens at a size you chose, and the layer bands keep their full share of the budget instead of competing with an unbounded transcript.
...
```ts
import { context, workingMemoryContext, type Context, type InferContext } from '@noetic-tools/core';
...
const snap = ctx.context['working-context'].snapshot; // WorkingMemoryContextState
await ctx.context['working-context'].update({ key: 'value' });
```

See [Custom Layers](/docs/framework/context-layers/custom-layers) for how to define your own `provides` entries using `layerData()` and `layerFn()`.

## Next Steps

- [Working Memory](/docs/framework/context-layers/working-memory-context) -- scratchpad for the current turn
- [Observational Context](/docs/framework/context-layers/observational-context) -- auto-extracted facts
- [Temporal Context](/docs/framework/context-layers/temporal-context) -- timestamped fact ledger with relative-time recall
- [Steering](/docs/framework/context-layers/steering) -- rule-based allow/deny/guide control over tool calls and responses
- [File Reference](/docs/framework/context-layers/file-reference) -- `#path` file tracking with priority-scored content injection
- [Static Content](/docs/framework/context-layers/static-content) -- load-once instructional content
- [History Window](/docs/framework/context-layers/history-window) -- cap items sent to the LLM
- [Custom Layers](/docs/framework/context-layers/custom-layers) -- build your own
- [Plan Context](/docs/framework/context-layers/plan-context) -- PRD authoring and plan execution lifecycle
- [Tool Context](/docs/framework/context-layers/tool-context) -- imperative state access and function-call state
