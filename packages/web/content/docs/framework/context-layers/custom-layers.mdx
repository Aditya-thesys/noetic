...
- **`projectHistory`** runs once per LLM step to project (cap, summarize, redact) the history portion of the context window. It is read-side only — `itemLog` storage is never mutated. See [History Window](/docs/framework/context-layers/history-window) for a layer built entirely from this hook.
...
Return `null` when you cannot summarise the change — the runtime republishes in full. The hook is only worth writing when the payload is large and its changes are small; `fileReference` uses it to send the one file that changed instead of the whole set.

## Step 7: Register with the Agent

Pass custom layers to a `react` pattern (or `spawn`'s `context` option):
...
```ts validate=none
import { context, workingMemoryContext, type InferContext } from '@noetic-tools/core';

const mem = context([workingMemoryContext(), myCustomLayer()]);
type Mem = InferContext<typeof mem>;

// Mem is:
// {
//   'working-context': { snapshot: WorkingMemoryContextState; update: (args: Record<string, unknown>) => Promise<void> };
//   'my-custom-layer': { entryCount: number; addEntry: (args: { text: string }) => Promise<void> };
// }
```

In a `step.run`, access the typed context:
...
- [Tool Context](/docs/framework/context-layers/tool-context) -- imperative state access via `toolCtx.context` and function-call state patterns
