---
title: Steering
description: A context layer that enforces behavioral rules before tool calls and after model responses, with programmatic and LLM-evaluated rules.
---
...
- **[Placement](/docs/framework/context-layers#prompt-cache-anchoring)**: `'live'` — `recall` drains the feedback queue as it renders, so the call cannot be replayed from a pin
