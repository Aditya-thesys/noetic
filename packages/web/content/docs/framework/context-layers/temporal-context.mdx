...
description: An LLM-backed context layer that distills the conversation into a ledger of timestamped facts and answers relative-time queries on demand.
---
...
- **[Placement](/docs/framework/context-layers#prompt-cache-anchoring)**: `'live'` while `groundDateTime` is on (the default) — the `<current_datetime>` block changes every turn. With `groundDateTime: false` it falls back to `'auto'`
- **Hook timeouts**: `store` and `onItemAppend` both `60000` ms (both run the LLM-backed extraction path)

## Usage

```ts validate=none
import { temporalContext } from '@noetic-tools/core';
...
```ts validate=none
interface TemporalContextConfig {
  now?: () => Date;
  scope?: ContextScope;
...
| `scope` | `ContextScope` | `'resource'` | Persistence boundary (long-term, cross-session by default) |
...
The layer provides one `layerFn`, exposed to the model as the `temporal/searchMemory` tool and to code as `ctx.context['temporal'].searchMemory({ query })`:
...
The layer is LLM-agnostic: `extract` and `search` are host-injected (mirroring `observationalContext`'s `observer`), keeping the context package tree-shakable. The code agent wires structured `step.llm` calls as the callbacks and installs `temporalContext()` in its default stack.
