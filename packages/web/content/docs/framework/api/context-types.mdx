...
| `abort` | `(reason?) => void` | Signal abort. Cascades to every live `fork` path and `spawn` child of this context (never to the parent); first call wins. Does not run memory-layer teardown — see `harness.cancel` |
