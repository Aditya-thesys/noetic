...
Collects an **array** of layers into a `ContextConfig` with inferred `_shape` — e.g. `context([workingMemoryContext(), planContext()])`. Pass the result to `StepSpawn.context` or `AgentConfig` to get typed `ctx.context` access.
