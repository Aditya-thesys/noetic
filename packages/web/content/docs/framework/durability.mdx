...
Returns `null` when no snapshot is recorded. Throws `NoeticConfigError` with `code: 'CHECKPOINT_SCHEMA_MISMATCH'` when the persisted `schemaVersion` is unknown — callers discard via `harness.clearCheckpoint(executionId)` and start fresh.
...
|---|---|---|
| `maxEntryBytes` | 128 KiB | The entry is not recorded. Keeps one write inside the per-value limit real backends impose instead of failing it. An output that cannot be JSON-encoded at all (a cycle, a `BigInt`) is treated the same way. |
| `maxEntries` | 1000 | The oldest entry is evicted, so resume is best-effort over a bounded **suffix** of the run: the tail replays, the head runs again. |
...
Replaying an output replays a step's **value**, not its **effect**. A `run` step that wrote a file, or a `tool` step that opened a PR, returns its recorded output on replay while the effect is not redone — correct only if that effect was durable when it happened.

Core's ledger therefore covers control flow and `llm` steps. It does **not** claim exactly-once execution for tools: it has no durable pre-dispatch record and no way to know whether a given tool is idempotent. Fence effects at the tool/host boundary.
...
The helper calls `harness.subprocess.listLive()`, then `harness.restore(executionId)` for every handle that carries an `executionId`. With no durable storage configured the call is a cheap no-op. A host that decorates its contexts re-applies that wiring per restored context — see [Restoring a decorated context](#restoring-a-decorated-context).
