...
| `execute` | `(args: I, toolCtx: ToolExecutionContext) => Promise<O>` | Yes | The function that runs the tool. The second argument is a `ToolExecutionContext` — `{ ctx, harness, fs, shell, memory, assembledView, lastStepMeta }` — not the step `Context` itself (which is available as `toolCtx.ctx`) |
| `needsApproval` | `boolean` | No | If `true`, the runtime should ask for human approval before executing |
| `memory` | `ToolMemoryDeclaration` | No | Declares tool-owned state; the runtime generates a memory layer from it |
