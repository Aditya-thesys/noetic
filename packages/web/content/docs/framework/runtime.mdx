...
const harness = new AgentHarness({
  ...
  initialStep: agent,
  params: {},
  llm: { provider: 'openrouter' }, // reads OPENROUTER_API_KEY from the env
});
...
| `initialStep` | `Step<TContext, string, string>` | `undefined` | The agent step to execute when calling `execute()`. `TContext` defaults to `ContextData`. |
| `params` | `TParams` | required | Arbitrary key-value parameters (populates `config.params`). |
| `context` | `ContextLayer[]` | `undefined` | Default context layers applied to every context created via `createContext()` / `execute()`. |
| `storage` | `StorageAdapter` | in-memory | Storage backend for context persistence. |
...
| `fs` | `FsAdapter` | `createLocalFsAdapter()` | Filesystem adapter exposed as `harness.fs`. Context layers, tools, and skill discovery all route through this adapter. |
...
| `itemSchemas` | `ItemSchemaExtensions` | `undefined` | Harness-wide item-schema extensions used to validate emitted/returned items. Global by design; tool-contributed `toolResults` schemas are owner-scoped to each tool's own result items. Mismatches raise `NoeticError` kind `item_schema_mismatch`. |
...
| `layerStateStore` | `LayerStateStore` | In-memory store | Backing store for context layer state. |
...
| `options` | `ExecuteOptions` | Optional `threadId`, `resourceId`, `state`, `context`, `deliveryMode`. |
...
| `context` | `ContextLayer[]` | Context layers for this context. Overrides harness-level `context` if provided. |
...
| `cancel(ctx, reason?)` | Cancel a running execution: abort `ctx` and every live descendant context (fork paths, spawn children), then run context-layer teardown — `onComplete` with `outcome: 'aborted'`, then `dispose` — bottom-up. A no-op on an already-cancelled context. |

Cancellation reaches inside the work in flight: blocked channel `recv`/`send` calls reject with `cancelled`, the current provider stream and sub-harness turn are cut short, and the next step boundary throws `cancelled`. Tokens and cost already spent stay charged to the context. Beyond those points it is cooperative — a `step.run` body that ignores `ctx.aborted` between `await`s runs to its next step boundary.
