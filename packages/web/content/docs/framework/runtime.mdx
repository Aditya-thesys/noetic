...
| `restore(executionId, opts?)` | Rebuild a `Context` from a previously saved snapshot. Returns `null` when no snapshot exists. Throws `NoeticConfigError` with `code: 'CHECKPOINT_SCHEMA_MISMATCH'` when the persisted `schemaVersion` is unrecognised. `opts` is a `RestoreContextOptions` (`parent` / `state` / `memory`) carrying back the host wiring a snapshot cannot serialise — see [Restoring a decorated context](/docs/framework/durability#restoring-a-decorated-context). |
| `cancel(ctx, reason?)` | Cancel a running execution: abort `ctx` and every live descendant context (fork paths, spawn children), then run memory-layer teardown — `onComplete` with `outcome: 'aborted'`, then `dispose` — bottom-up. A no-op on an already-cancelled context. |
...
Use [`ctx.abort(reason)`](/docs/framework/context#abort) for the same signal without layer teardown (it is synchronous and returns `void`); use `cancel` when the harness owns the memory lifecycle. Note that `harness.abort(scope?)` is a different operation: it cancels the current *session turn* by thread, not a specific context tree.
