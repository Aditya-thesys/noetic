...
The layer exposes two functions to the model as tools, so an agent can report what it produced. They are also callable from code via `ctx.memory['durable-task-state']`.
