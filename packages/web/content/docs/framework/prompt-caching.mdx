...
Worth implementing only when the payload is large and its changes are small. The built-in `fileReference` layer does it: one changed file out of twelve costs one file, not twelve.
