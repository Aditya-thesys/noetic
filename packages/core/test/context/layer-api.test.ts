...
const COUNTER_SCOPE: ContextScope = 'execution';
...
function makeEmptyLayer(): ContextLayer {
  return {
    id: 'empty',
    slot: Slot.OBSERVATIONS,
    scope: 'execution' satisfies ContextScope,
    hooks: {},
  };
}
...
describe('ctx.context', () => {
  it('reads data from layer state via ctx.context[layerId]', () => {
    ...
    expect(ctx.context.counter.value).toBe(42);
  });
  ...
  it('data reads are live — reflect state changes', () => {
    ...
    expect(ctx.context.counter.value).toBe(1);

    harness.setLayerState(ctx.id, 'counter', {
      count: 99,
    });
    expect(ctx.context.counter.value).toBe(99);
  });
  ...
  it('returns empty handle for layer without provides', () => {
    ...
    expect(Object.keys(ctx.context.empty)).toEqual([]);
  });
  ...
  it('memory is empty object when no layers', () => {
    ...
    expect(Object.keys(ctx.context)).toEqual([]);
  });
